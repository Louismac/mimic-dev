 ...
 */
  addRegression(
      ...
      )
   {
      ...
      {
        this.y.push(0);
        this.addSmoothing(smoothOutput);
      }
      if(gui)
      {
  ...
  */
  ...
  runEnd(data) {
    if(this.onOutput !== undefined)
    {
      ...
      {
        let output = data[i];
        if(this.smoothingBuffers[i] !== undefined)
        {
          this.smoothingBuffers[i].push(output)
          output = this.smoothingBuffers[i].mean();
          if(this.modelType == 0)
          {
  ...
  */
      ...
      newFeatures.forEach((f)=>{
        ...
        inputs.forEach((i)=>{
          ...
          f.features.forEach((fn, j)=>{
            ...
            //If raw just use rms with window of 1 (same)
            if(fn === "raw") {
              window = 1
              fn = "rms"
            }
            buffers.push({
              buf:new this.rapidLib.StreamBuffer(window),
              fn:fn,
            })
          })
          this.features.push({input:i,buffers:buffers})
        });
      })
    } catch (err) {
  ...
  resetFeatures() {
    if(this.features !== undefined) {
      this.features.forEach((pipeline)=>{
        pipeline.buffers.forEach((buffer)=>{
          buffer.buf.reset();
        })
      })
    }
  ...
  getFeatures(input) {
    ...
    //For every pipeline
    this.features.forEach((pipeline)=> {
      let val = input[pipeline.input]
      //Chain the input through the features
      pipeline.buffers.forEach((b)=>{
        b.buf.push(val)
        val = b.buf[b.fn]()
      })
      //Save as new output
  ...
  addSmoothing(w)
  {
    try {
      if(w > 0)
      {
        ...
        this.smoothingBuffers.push(new this.rapidLib.StreamBuffer(w))
      }
    } catch (err) {
      this.onRapidLoad.push(()=>{this.addSmoothing(w)})
    }
  }
  ...
  downloadTrainingData() {
    return new Promise((resolve, reject)=> {
      this.trainingData().then((res)=>{
        this.downloadObjectAsJson(res, "myLearnerData")
        resolve();
      });
    });
  }
